...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path == "/favicon.ico":
            ...
            return
        if self.path == "/login":
            self.send_raw(200, LOGIN_HTML)
            return
        if not self.require():
            return
        if self.path == "/":
            self.send_raw(200, DASH_HTML)
